...
export function ClassroomJoinModal({ progress, onClose, onOpenTeacherLogin, prefillCode }: ClassroomJoinModalProps) {
  ...
  return (
    // Scrollable overlay + m-auto card (instead of items-center/justify-center): centered when it
    // fits, but when it doesn't (phone keyboard open, recovery list) it scrolls from the top rather
    // than clipping the card's header off-screen with no way to reach it.
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex overflow-y-auto p-4">
      <div className="m-auto bg-white rounded-3xl shadow-2xl max-w-md w-full border border-slate-200 p-5 sm:p-6 space-y-5">
        <div className="flex items-start justify-between">
          ...
          {canDismiss && (
            <button onClick={onClose} aria-label="ปิด" className="p-2.5 -m-2.5 text-slate-400 hover:text-slate-600">
              <X className="w-5 h-5" />
            </button>
          )}
        </div>
        ...
        {status === 'confirm-recovery' ? (
          <div className="space-y-3">
            ...
            <div className="space-y-2 max-h-56 overflow-y-auto">
              {recoveryMatches.map((m) => (
                <button
                  ...
                  className="w-full text-left px-3 py-3.5 sm:py-3 bg-indigo-50 border border-indigo-100 rounded-xl hover:border-indigo-300 transition-colors"
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs font-bold text-indigo-900 flex items-center gap-1.5">
                      <UserCheck className="w-3.5 h-3.5" /> ใช่ นี่คือฉัน
                    </span>
                    <span className="text-xs sm:text-[11px] font-bold text-indigo-600">{m.progress.xp} XP</span>
                  </div>
                  <p className="text-xs sm:text-[10px] text-indigo-500 mt-1">
            ...
            <button
              ...
              className="w-full min-h-11 sm:min-h-0 py-2.5 rounded-xl text-sm sm:text-xs font-bold text-slate-500 hover:bg-slate-50 border border-slate-200"
            >
          ...
          <>
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                ...
                <input
                  ...
                  className="mt-1 w-full px-3 py-3 sm:py-2.5 border border-slate-200 rounded-xl text-base sm:text-sm focus:outline-none focus:border-indigo-400"
                />
              </div>
              <div>
                <label className="text-xs sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                ...
                <input
                  ...
                  maxLength={8}
                  className="mt-1 w-full px-3 py-3 sm:py-2.5 border border-slate-200 rounded-xl text-base sm:text-sm tracking-widest font-bold focus:outline-none focus:border-indigo-400"
                />
              </div>
              ...
              {existingLink && status === 'idle' && (
                <div className="flex items-center justify-between text-xs sm:text-[11px] text-slate-500 bg-slate-50 rounded-xl px-3 py-2">
                  <span>
                    ตอนนี้เชื่อมกับห้อง <strong className="text-slate-700">{existingLink.classCode}</strong> อยู่
                  </span>
                  <button onClick={handleLeaveClass} className="min-h-11 sm:min-h-0 px-2 -mr-2 text-rose-500 font-bold hover:underline">
                    ยกเลิก
                  </button>
                </div>
              )}
            ...
            <div className="flex gap-2 pt-2">
              {canDismiss && (
                <button
                  onClick={onClose}
                  className="flex-1 min-h-11 sm:min-h-0 py-2.5 rounded-xl text-sm sm:text-xs font-bold text-slate-500 hover:bg-slate-50 border border-slate-200"
                >
                  ปิด
                </button>
              )}
              <button
                ...
                className="flex-1 min-h-11 sm:min-h-0 py-2.5 rounded-xl text-sm sm:text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 flex items-center justify-center gap-1.5"
              >
        ...
        {!canDismiss && onOpenTeacherLogin && (
          <button
            ...
            className="w-full min-h-11 sm:min-h-0 text-center text-xs sm:text-[11px] text-slate-400 hover:text-slate-600 font-bold flex items-center justify-center gap-1.5 sm:pt-1"
          >
