  ...
  if (notFound) {
    return (
      <div className="max-w-md mx-auto text-center space-y-3 py-16">
        ...
        <p className="text-sm font-bold text-slate-600">ไม่พบคำถามนี้ หรือคำถามถูกลบไปแล้ว</p>
        <p className="text-xs text-slate-400">ลองขอ QR หรือลิงก์ใหม่จากครูอีกครั้ง</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-md mx-auto space-y-4 py-6">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
        ...
        <p className="text-base font-bold text-slate-800 leading-relaxed">{poll.question}</p>

        {poll.status === 'closed' ? (
          <p className="text-sm font-bold text-slate-500 bg-slate-50 rounded-xl p-4 text-center">
          ...
          <div className="space-y-2">
            {poll.options.map((opt) => (
              <button
                ...
                disabled={submitting}
                className={`w-full text-left px-4 py-3 rounded-xl border text-sm font-bold transition-colors disabled:opacity-60 ${
                  selected === opt
        ...
        {selected && poll.status === 'open' && (
          <p className="text-xs font-bold text-emerald-600 flex items-center gap-1.5">
            <CheckCircle2 className="w-4 h-4" /> ส่งคำตอบแล้ว รอครูเฉลย (แตะข้อใหม่เพื่อเปลี่ยนคำตอบได้)
          </p>
        )}
      </div>
    </div>
