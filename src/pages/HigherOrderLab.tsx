    ...
    <div className="space-y-5 pb-12 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className={`px-3 py-1.5 min-h-11 sm:min-h-0 rounded-xl text-xs font-bold border transition-all ${
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-sm">
        ...
        {/* Fixed-size cells in a grid/row that must never shrink: with minmax(0, 1fr) and a
            shrinkable grid, a phone-width card compressed the grid to ~100px while each input
            stayed 48px wide, so the cells overlapped. Below sm the cells get smaller and the
            x/y/z/w column moves to headers above A so the whole row fits a 375px screen; the
            *-center-safe alignment makes anything narrower scroll instead of clipping its left edge. */}
        <div className="flex items-center justify-center-safe gap-2 sm:gap-3 py-2 overflow-x-auto">
          <span className="text-3xl font-light text-slate-300">[</span>
          <div
            className="grid gap-1.5 sm:gap-2 flex-shrink-0"
            style={{ gridTemplateColumns: 'repeat(4, minmax(0, 1fr))' }}
          >
            {VARS.map((v) => (
              <div key={v} className="sm:hidden text-center font-bold text-indigo-700 text-sm">
                {v}
              </div>
            ))}
            {matrixStrA.map((row, r) =>
              row.map((val, c) => (
                <input
                  ...
                  className="w-10 h-10 sm:w-12 sm:h-12 text-center font-bold text-base sm:text-sm rounded-lg border border-slate-200 bg-slate-50 focus:outline-none focus:border-indigo-400 focus:bg-white"
                />
              ))
            )}
          </div>
          <span className="text-3xl font-light text-slate-300">]</span>
          <div className="hidden sm:flex flex-col gap-2 ml-2 flex-shrink-0">
            {VARS.map((v) => (
          ...
          {/* pt-[26px] below sm lines the B cells up with A's rows under the x/y/z/w header row. */}
          <div className="flex flex-col gap-1.5 sm:gap-2 flex-shrink-0 pt-[26px] sm:pt-0">
            {vectorStrB.map((val, r) => (
              <input
                key={r}
                value={val}
                onChange={(e) => handleBChange(r, e.target.value)}
                className="w-10 h-10 sm:w-12 sm:h-12 text-center font-bold text-base sm:text-sm rounded-lg border border-amber-200 bg-amber-50 focus:outline-none focus:border-amber-400 focus:bg-white"
              />
            ))}
          </div>
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-xs">
          ...
          <button
            ...
            className="px-4 py-2 min-h-11 sm:min-h-0 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-sm flex items-center gap-1.5"
          >
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-sm space-y-3">
          ...
          {gaussSteps.slice(0, revealedCount).map((gs, idx, revealed) => {
            ...
            return (
              <div
                key={gs.stepIndex}
                className={`p-3 sm:p-4 rounded-2xl border text-xs space-y-1 transition-all ${
          ...
          {isWalkthroughComplete ? (
            ...
            <button
              ...
              className="px-4 py-2 min-h-11 sm:min-h-0 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-sm flex items-center gap-1.5 mx-auto"
            >
